...
input.forEach((pairing) => {
  if (pairingsFullyOverlap(pairing)) {
    counter++
  }
})

function pairingsFullyOverlap(pairing: Range[]): boolean {
  return (
    (pairing[0].min >= pairing[1].min && pairing[0].max <= pairing[1].max) ||
    (pairing[1].min >= pairing[0].min && pairing[1].max <= pairing[0].max)
  )
}
